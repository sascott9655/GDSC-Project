...
```{r}
set.seed(seed)
knn_baseline <- train(trainEncodedPredictors, trainIC50,
             method = "kknn",
             tuneGrid = data.frame(
             kmax = c(3, 7, 11), 
             distance = 2,            
...
```{r}
knn_Imp <- varImp(knn_baseline, scale = FALSE)
plot(knn_Imp, top = 25)
```
...
```{r}
set.seed(seed)
svmGrid_base <- expand.grid(
                sigma = c(0.001, 0.01, 0.1),
                C = c(1, 10))
SVM_baseline <- train(trainEncodedPredictors, trainIC50,
             method = "svmRadial",
             tuneGrid = svmGrid_base,
             preProcess = c("center", "scale"),
             trControl = threefoldCV) 
SVM_baseline
```
...
```{r}
SVM_Imp <- varImp(SVM_baseline, scale = FALSE)
plot(SVM_Imp, top = 25)
```

```{r}
set.seed(seed)
MARSGrid_base <- expand.grid(
                  degree = 1,
                  nprune = seq(2, 20, length = 10))
MARS_baseline <- train(trainEncodedPredictors, trainIC50,
              method ="earth",
              tuneGrid = MARSGrid_base,
              trControl = threefoldCV)
MARS_baseline
```

```{r}
MARS_Imp <- varImp(MARS_baseline, scale = FALSE)
plot(MARS_Imp, top = 25)
```

```{r}
set.seed(seed)
mtryGrid_base <- data.frame(mtry = c(2, 4, 6, 8, 10))
rf_baseline <- train(trainEncodedPredictors, trainIC50,
                method = "rf",
                tuneGrid = mtryGrid_base,
                ntree = 500,
                importance = FALSE,
                trControl = threefoldCV
            )
...
```{r}
rf_Imp <- varImp(rf_baseline, scale = FALSE)
plot(rf_Imp, top = 25)
```
...
train_cell_lines <- unique(trainPredictors$CELL_LINE_NAME)
test_cell_lines <- unique(testPredictors$CELL_LINE_NAME)

reduced_cell_lines_tr <- sample(
  train_cell_lines,
  size = floor(row_reduction * length(train_cell_lines))
)

reduced_cell_lines_tst <- sample(
  test_cell_lines,
  size = floor(row_reduction * length(test_cell_lines)) 
)

reducedRows_tr <- trainPredictors$CELL_LINE_NAME %in% reduced_cell_lines_tr

trainReducedRows <- trainReduced[reducedRows_tr,]

trainIC50ReducedRows <- trainIC50[reducedRows_tr]

reducedRows_tst <- testPredictors$CELL_LINE_NAME %in% reduced_cell_lines_tst

testReducedRows <- testReduced[reducedRows_tst,]
testIC50ReducedRows <- testIC50[reducedRows_tst]
...
MARS <- train(trainReducedRows, trainIC50ReducedRows,
              method ="earth",
              tuneGrid = MARSGrid,
              trControl = fivefoldCV)
MARS
...
testResults_MARS <- predict(MARS, testReduced2)

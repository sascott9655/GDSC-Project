...
#naturally encoding all your categorical variables to be dummy variables, it was 
#computationally expensive. Instead predictors like CELL_LINE_NAME and
#DRUG_NAME are removed from the model to #decrease computation time and they
#did not provide predictive power.
...
results_table <- data.frame(
  Model = c("Linear Regression (lin_reg)",
            "KNN Model (knn)",
            "Support Vector Machine (SVM)",
            "MARS model (MARS)",
            "Random Forest (rf)"),
  RMSE = c(2.0396, 2.1946, 2.0511, 2.0396, 2.0397),
  R2   = c(0.4481, 0.4345, 0.4450, 0.4481, 0.4480),
  MAE  = c(1.5993, 1.6798, 1.5966, 1.5993, 1.5997)
)

table_img <- results_table %>%
  gt() %>%
  tab_header(title = "Model Performance Comparison") %>%
  fmt_number(columns = c(RMSE, R2, MAE), decimals = 4) %>%
  cols_align(align = "center", columns = c(RMSE, R2, MAE)) %>%
  tab_style(
...
#gtsave(table_img, "model_performance.png") Created an image for report.
```
